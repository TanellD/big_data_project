#!/usr/bin/env python3
"""
Database setup script for the project to process Airbnb data
...
def get_connection_string():
    ...
    try:
        ...
        with open(os.path.join("secrets", ".psql.pass"), "r",
        ...
        print("Password file not found. Please create secrets/.psql.pass " +
...
def preprocess_data():
    ...
    data = pd.read_csv(os.path.join("data", "data.csv"), low_memory=False,
                       sep=';')
    ...
    for col in hosts.select_dtypes(include=['object']).columns:
        hosts[col] = (hosts[col].astype(str)
                      .str.replace(r'\r|\n', ' ', regex=True)
                      .str.replace('nan', ''))

    # Review scores
    review_scores = data[[
        'ID', 'Review Scores Rating', 'Review Scores Accuracy',
        'Review Scores Cleanliness', 'Review Scores Checkin',
        'Review Scores Communication', 'Review Scores Location',
        'Review Scores Value'
    ]].copy()
    review_scores.columns = [col.lower().replace(' ', '_')
                             for col in review_scores.columns]
    ...
    for col in review_scores.select_dtypes(include=['object']).columns:
        review_scores[col] = (review_scores[col].astype(str)
                              .str.replace(r'\r|\n', ' ', regex=True)
                              .str.replace('nan', ''))
    ...
    listings = data[[
        ...
        'Number of Reviews', 'First Review', 'Last Review', 'License',
        'Jurisdiction Names', 'Cancellation Policy',
        'Calculated host listings count', 'Reviews per Month',
        # Location fields
        'Street', 'Neighbourhood', 'Neighbourhood Cleansed',
        'Neighbourhood Group Cleansed', 'City', 'State', 'Zipcode',
        'Market', 'Smart Location', 'Country Code',
        'Country', 'Latitude', 'Longitude', 'Geolocation'
    ]].copy()

    listings.columns = [col.lower().replace(' ', '_')
...
def copy_dataframe_to_table(cursor, dataframe, table_name):
    ...
    for col in date_columns:
        # Convert to datetime objects first (will set invalid dates to NaT)
        df_copy[col] = pd.to_datetime(df_copy[col], errors='coerce')
        # Format valid dates as strings, leave NaT as None
        df_copy[col] = df_copy[col].apply(lambda x: x.strftime('%Y-%m-%d')
                                          if pd.notna(x) else None)

    # For non-date columns, replace empty strings with None
    for col in df_copy.columns:
        if col not in date_columns:
            df_copy[col] = df_copy[col].apply(lambda x: None
...
def main():
    ...
    try:
        with psql.connect(conn_string) as conn:
            ...
            tables_to_import = ['hosts', 'listings',
                                'review_scores', 'listing_features']
